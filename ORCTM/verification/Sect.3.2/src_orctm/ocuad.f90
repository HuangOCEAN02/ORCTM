...
!
      ...
      USE MO_UNITS
!
      DIMENSION TRF0(1:IE-I_start+1,JE,KE)

      DIMENSION TRP(I_start:IE,1:JE,1:KEP),TRM(I_start:IE,1:JE,0:KE)
      DIMENSION TRF(I_start:IE,1:JE,1:KE),TRVOL(I_start:IE,1:JE,1:KE)
      DIMENSION TR1(I_start:IE,1:JE,1:KE)
      DIMENSION WTP(I_start:IE,1:JE,1:KEP),WTM(I_start:IE,1:JE,0:KE)
...
!$OMP DO
       ...
       UP=0.5*DT*DLYP(I+1,J)*(UOS+ABS(UOS))
       UM=0.5*DT*(ABS(UWE)-UWE)*DLYP(I,J)
       SCHA=MAX(0.,(ABL-ZWABL)/(ABL+1.E-20))
!
       SCH=MIN(1.,SCHA*TRVOL(I,J,K)/(UP+UM+1.E-20))
       SCHI=1.-SCH
...
3      CONTINUE
      ...
      IF (have_g_is) THEN
       ...
       DO 31 J=2,JE1
       UOS=0.5*(UKO(I,J,K)*DDUO(I,J,K)+UKO(I+1,J,K)*DDUO(I+1,J,K))
       UWE=UKO(I,J,K)*DDUO(I,J,K)
       UP=0.5*DT*DLYP(I+1,J)*(UOS+ABS(UOS))
       UM=0.5*DT*(ABS(UWE)-UWE)*DLYU(I,J)
...
!$OMP DO
       ...
       UP=0.5*DT*(0.5*(DLXV(I,J-1)+DLXV(I+1,J-1)))*(VNO+ABS(VNO))
       UM=0.5*DT*(0.5*(DLXV(I,J)+DLXV(I+1,J)))*(ABS(VSU)-VSU)
!
       SCHA=MAX(0.,(ABL-ZWABL)/(ABL+1.E-20))
       SCH=MIN(1.,SCHA*TRVOL(I,J,K)/(UP+UM+1.E-20))
!
