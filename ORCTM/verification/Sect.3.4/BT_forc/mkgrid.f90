     ...
     & U10OUT,RIV
      ...
      integer(i8),parameter::ie=1004,je=5,ito=ie*2,jto=je*2,ke=150
...
! grid ----- anta
      ...
      DO J=1,JE
       DO I=1,IE
        depto(I,J)=dep
        if ((j.GE.3) .AND. (j.LE.(je-2))) then 
             if ( i.GE.680 ) then
                depto(I,J)= depto(I,J) - 0.214*(i-680)*ddx
             endif
        endif
...
! TS ----- INI*
      ...
      end program ISW_BREAKING_SLOPE
