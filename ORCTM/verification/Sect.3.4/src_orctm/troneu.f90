...
!
     ...
     &- ( V1E(I,J-1) *CONO+CONN*VZE(I,J-1))*DLXV(I,J-1))/DLXP(I,J)  )
      ...
      IF (have_g_is) THEN
        ...
        ENDDO
        ! write(IO_STDOUT,*) 'TRONEU West'
        ! write(IO_STDOUT,*) 'Z1O(1,:)',Z1O(1,:)
        ! write(IO_STDOUT,*) 'B10(1,:)',B1O(1,:)
     ENDIF
      IF (have_g_ie) THEN
        DO J=1,JE
          Z1O(IE,J)  = B1O(IE,J)/XX(IE,J)
        ENDDO
       ! write(IO_STDOUT,*) 'TRONEU East'
       ! write(IO_STDOUT,*) 'Z1O(IE-2:IE,:)',Z1O(IE-2:IE,:)
       ! write(IO_STDOUT,*) 'B10(IE-2:IE,:)',B1O(IE-2:IE,:)
      ENDIF
      IF (have_g_js) THEN
        DO I=1,IE
          Z1O(I,1)  = B1O(I,1)/XX(I,1)        
        ENDDO
        !write(IO_STDOUT,*) 'TRONEU North'
        !write(IO_STDOUT,*) 'Z1O(:,1:2)',Z1O(:,1:2)
        !write(IO_STDOUT,*) 'B10(:,1:2)',B1O(:,1:2)
      ENDIF
      IF (have_g_je) THEN
        DO I=1,IE
          Z1O(I,JE)  = B1O(I,JE)/XX(I,JE)
        ENDDO
       ! write(IO_STDOUT,*) 'TRONEU South'
       ! write(IO_STDOUT,*) 'Z1O(:,JE-2:JE)',Z1O(:,JE-2:JE)
       ! write(IO_STDOUT,*) 'B10(:,JE-2:JE)',B1O(:,JE-2:JE)
      ENDIF
#endif
