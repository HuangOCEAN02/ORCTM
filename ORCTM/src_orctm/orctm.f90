...
!
!--------------------------------------------------------------------
!  OBCS : SEA SURFACE LEVEL FORCINGS  ---- ETA
#ifdef OBC_ETA_FORC
...
!
!--------------------------------------------------------------------
!  OBCS : BOUNDARY BT TIDAL FORCINGS ---- UV
...
!
#ifdef MEAN
       IF ( MOD( LDTRUN,NINT(900/DT) ) .EQ.  1 ) THEN ! Global output interval: 15 min  
        CALL WRTE_MEAN(LDTDAY,LDAYS,LMONTS,LYEARS,IMEAN,NANF)
       ENDIF
#ifdef DIAGNOSIS
       IF ( MOD( LDTRUN,NINT(60/DT) ) .EQ.  1 ) THEN ! Diagnostic output interval: 1 min  
        CALL WRTE_DIAGNOSIS(LDTDAY,LDAYS,LMONTS,LYEARS,DMEAN,NANF)
       ENDIF
#endif /*DIAGNOSIS*/     
       if (LDTRUN .GE. NINT((86400*10)/DT) )  then ! ISWs simulation period: 10 days 
#ifndef FRAGMENT
          CALL AUFW
#else
          CALL AUFW_FRAGMENT
