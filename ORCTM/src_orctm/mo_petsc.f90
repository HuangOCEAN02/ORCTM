...
! The land ponits do not need to discretized.        
        ...
        ELSE
! The homogeneous Neumann Boundary Conditions (dpnh/dx=0) are accepted for the land boundaries
          ! west/east boundary
          IF     ( weto(I-1,J,K).EQ.0 .AND. weto(I+1,J,K).EQ.1 ) THEN
             X_W=0.0
             X_C=XU(I,J)*XP(I,J)
...
! Open Boundary Conditions for Solving Poission Problem: pnh=0 at OB
