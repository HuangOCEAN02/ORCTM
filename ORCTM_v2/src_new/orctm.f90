...
!    ORCTM :           Nov 20 2020  SPY     - update the Open Boundary Module(Sponge Relaxation+Orlanski)
!                      FRI 20 2021  GJQ     - update the Open Boundary Module(FLA+Orlanski) 
!                      FEB 16 2021  H.H     - update the Open Boundary Module(Purely Sponge Relaxation) 
!                      JUL 03 2021  QS      - update the Open Boundary Module(Volume Balance)
!                                           - created mo_obcs.f90 above all
!                      MAR 21 2021  H.H     - update the N-S Periodic boundary conditions
!                      JUL 05 2021  SPY&H.H - update the Nonhydrostatic Module(3D/2D-Possion)
!                                           - created mo_petsc.f90 & ocvtot_nh.f90
!*************************************************************************
!
!
!                       MP ORCTM
...
!
!HH    IO_IN_Z370 Z37000  RESTART-INFORMATION   ORCTM.F90,SBR AUFR,SBR AUFW
!HH    IO_IN_Z380 Z38000  RESTART-INFORMATION   ORCTM.F90,SBR AUFR,SBR AUFW
!HH    IO_IN_INIT INITEM  TEMPERATURE LEVITUS          ORCTM.F90,SBR LEVIRE
!HH    IO_IN_INIS INISAL  SALINITY LEVITUS             ORCTM.F90,SBR LEVIRE
!HH    IO_IN_SURS SURSAL  SEA SURFACE SALINITY LEVITUS ORCTM.F90,SBR LEVIRE
!
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!
!
...
!
      IF(p_pe==p_io) THEN
        OPEN(IO_IN_ARCG,FILE='./BT_forc/arcgri',                                  &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
!
        OPEN(IO_IN_INIT,FILE='./BT_forc/INITEM',STATUS='UNKNOWN',               &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
        OPEN(IO_IN_INIS,FILE='./BT_forc/INISAL',STATUS='UNKNOWN',               &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
        IF (RELSAL .GT. ALMZER) THEN
        OPEN(IO_IN_SURS,FILE='./BT_forc/SURSAL',STATUS='UNKNOWN',               &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
        ENDIF
        IF (RELTEM .GT. ALMZER) THEN
        OPEN(IO_IN_SURT,FILE='./BT_forc/SURTEM',STATUS='UNKNOWN',               &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
        ENDIF
!
        OPEN(IO_IN_GWIX,FILE='./BT_forc/GIWIX',STATUS='UNKNOWN',                &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
        OPEN(IO_IN_GWIY,FILE='./BT_forc/GIWIY',STATUS='UNKNOWN',                &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
        OPEN(IO_IN_GTEM,FILE='./BT_forc/GITEM',STATUS='UNKNOWN',                &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
        OPEN(IO_IN_GPRE,FILE='./BT_forc/GIPREC',STATUS='UNKNOWN',               &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
        OPEN(IO_IN_GSWR,FILE='./BT_forc/GISWRAD',STATUS='UNKNOWN',              &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
        OPEN(IO_IN_GTDE,FILE='./BT_forc/GITDEW',STATUS='UNKNOWN',               &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
        OPEN(IO_IN_GU10,FILE='./BT_forc/GIU10',STATUS='UNKNOWN',                &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
        OPEN(IO_IN_RVAL,FILE='./BT_forc/GIRIV',STATUS='UNKNOWN',                &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
#ifdef FORCE_SLP
        OPEN(IO_IN_GSLP,FILE='./BT_forc/GIPRESS',STATUS='UNKNOWN',              &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
#endif
#ifdef FORCE_DWLW
        OPEN(IO_IN_GLWR,FILE='./BT_forc/GIDWLW',STATUS='UNKNOWN',               &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
#else
        OPEN(IO_IN_GCLO,FILE='./BT_forc/GICLOUD',STATUS='UNKNOWN',              &
     &          ACCESS='SEQUENTIAL',FORM='UNFORMATTED')
#endif /*FORCE_DWLW*/
!
#ifdef FRAGMENT
...
#ifdef GLOBAL_NETCDF
     call system("mkdir -p tho")
     call system("mkdir -p sao")
     call system("mkdir -p uoo")
     call system("mkdir -p voe")     
     call system("mkdir -p poo")      
     call system("mkdir -p zoo")     
     call system("mkdir -p woo") 
#ifdef BAROTROPIC
     call system("mkdir -p uso")
     call system("mkdir -p vse") 
#endif
#ifdef DENSITYIO
     call system("mkdir -p roo") 
#endif 
#ifdef NON_HYDROSTATIC
     call system("mkdir -p pnh")
#endif
#ifdef DIFFDIAG
     call system("mkdir -p avo")
     call system("mkdir -p dvo")
#endif
...
!    START FROM RESTART OBCS OBC37 OR OBC38
      IF (IAUFR .EQ. 1) THEN
        Z3DOC = 3 ! for read_slice      
#ifndef FRAGMENT
        CALL AUFR  
#else
        ! CALL AUFR  !for nested simulation from the whole SCS to LV-Strait
        CALL AUFR_FRAGMENT
#endif /*FRAGMENT*/ 
#ifdef ORLANSKI
        ! For restart-files about Olanski radiation scheme
        CALL AUFR_OBCS
#endif /*ORLANSKI*/ 
        DO J=1,JE
          DO I=1,IE
...
!:: ORCTM-NH:: SCS-ISW Simulation with Background Current
       LREADMAX=0
       LREAD=0
!
#ifdef NOSKIP
...
!--------------------------------------------------------------------
! #ifdef OBCS_TST_NEST
     ! CALL OCOBCSBT(LDTRUN)
! #endif
      CALL OCBARP
      ! upgrade UZO & VZE
      ! write(IO_STDOUT,*) 'OCBARP', LDTRUN
!--------------------------------------------------------------------
...
#endif
      CALL OCVTRO
      ! Return USO & VSE and OB values upgrade by FLA
!--------------------------------------------------------------------
!
#ifdef TIMECHECK
      ttt = MPI_Wtime()
      write(0,123)'Time OCVTRO & OBCS',ttt-tttr
#endif
!--------------------------------------------------------------------
! OBCS FOR VERTICAL VELOCITY ADDED INTO OCVTOT
! FOR HY & NON-HY 
!  Updated by H.H on 12.04.2023 in Kiel Germany
#ifdef TIMECHECK
      tttr = MPI_Wtime()
#endif
      CALL OCVTOT
...
#endif
!  FOR TRACER OF APPLY OBC 
!  Updated by H.H on 12.04.2023 in Kiel Germany
#ifdef OBC_TS_FORC
     !  CALL UPDATE_ORLANSKI_TS(THO,SAO) 
     !  CALL OBCS_APPLY_TS
#endif
...
#ifdef TIMECHECK
       ttt = MPI_Wtime()
       write(0,123)'Time ORLANSKI',ttt-tttr
#endif
!
       CALL OCTIMF
!
#ifdef MEAN
       IF ( MOD( LDTDAY,NINT(3600/DT) ) .EQ.  1 ) THEN
!       IF ( MOD( LDTRUN,NINT(120/DT) ) .EQ.  1 ) THEN
#ifdef GLOBAL_NETCDF
         NITER = LDTRUN - 1
#endif        
       ...
       ENDIF
#ifdef DIAGNOSIS
       !IF ( MOD( LDTRUN,NINT(60/DT) ) .EQ.  1 ) THEN
        CALL WRTE_DIAGNOSIS(LDTDAY,LDAYS,LMONTS,LYEARS,DMEAN,NANF)
       !ENDIF
#endif /*DIAGNOSIS*/     

!       if (LDTRUN .GE. NINT((86400*15)/dt) )  then  ! one day
!       if (LDTRUN .GE. 300)  then ! all 10 tidal period
#ifndef FRAGMENT
!          CALL AUFW
#else
!          CALL AUFW_FRAGMENT
