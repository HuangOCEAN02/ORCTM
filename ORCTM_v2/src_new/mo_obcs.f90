      ...
      USE mo_parallel
      USE mo_units
! Main Switch for Open Boundary which should be defined firstly !!!
      LOGICAL:: West = .TRUE., East = .FALSE.
      LOGICAL:: North = .TRUE., South = .TRUE.
...
! Sponge Forcing Condition Switch for total flow or Elevation
      LOGICAL:: SpongeWest = .TRUE., SpongeEast = .FALSE.
      LOGICAL:: SpongeNorth = .TRUE., SpongeSouth = .TRUE.
!  Switch for TST-Boundary 
      LOGICAL:: TSTWest = .FALSE.,TSTEast = .FALSE.
      LOGICAL:: TSTNorth = .FALSE., TSTSouth = .FALSE.
!  Switch for Radiation Boundary for tangential velocity
      LOGICAL:: TanWest = .FALSE., TanEast = .FALSE.
      LOGICAL:: TanNorth = .FALSE., TanSouth = .FALSE.
!  Switch for Champman Open Boundary for BT velocty
      LOGICAL:: ChpmWest = .FALSE., ChpmEast = .FALSE.
      LOGICAL:: ChpmNorth = .FALSE., ChpmSouth = .FALSE.
!  Swith for corners values average for Open Boundary Corners
      LOGICAL:: Cornersmean = .FALSE.
! whether tidal wave applies to the 2D-Sponge Forcing Condition(BT flow)
      LOGICAL::add_tide = .FALSE.
! Partial tide numbers
      INTEGER,PARAMETER::tidalnumber = 8
...
! Sponge layer time parameters
      REAL:: rel_ui, rel_ub, rel_vi, rel_vb
...
! tidal harmonic constants
     ...
     &         obc_zs(:),obc_us(:,:),obc_vs(:,:),obc_uzs(:),obc_vzs(:)
	    ...
	    REAL, POINTER :: obc_usw(:,:),obc_vsw(:,:),obc_use(:,:),obc_vse(:,:), &
     &         obc_usn(:,:),obc_vsn(:,:),obc_uss(:,:),obc_vss(:,:)
...
#ifdef OBC_SUBTIDE
! Add for subtidal flow, QS
      INTEGER,PARAMETER::                                                   &
     & IO_IN_OBCZRW=916,IO_IN_OBCZRE=926,IO_IN_OBCZRN=936,IO_IN_OBCZRS=946, &
     & IO_IN_OBCURW=917,IO_IN_OBCURE=927,IO_IN_OBCURN=937,IO_IN_OBCURS=947, &
     & IO_IN_OBCVRW=918,IO_IN_OBCVRE=928,IO_IN_OBCVRN=938,IO_IN_OBCVRS=948
#endif
...
! allocate memory for OBCS module
     ...
     &            cl_ts(ie,       ke),   cl_ss(ie,       ke) )
      ...
      allocate(  obc_usw(je,ke),obc_vsw(J_start:je,ke), &
     &           obc_use(je,ke),obc_vse(J_start:je,ke), &
     &           obc_usn(I_start:ie,ke),obc_vsn(ie,ke), &
     &           obc_uss(I_start:ie,ke),obc_vss(ie,ke) )
      ...
      rel_ui=0.0;rel_ub=0.0;rel_vi=0.0;rel_vb=0.0
      ...
      obc_usw(:,:)=0.0;obc_vsw(:,:)=0.0
      obc_use(:,:)=0.0;obc_vse(:,:)=0.0
      obc_usn(:,:)=0.0;obc_vsn(:,:)=0.0
      obc_uss(:,:)=0.0;obc_vss(:,:)=0.0
...
#ifdef OBC_TIDE
       IF( West ) THEN
        OPEN(IO_IN_OBCZW,FILE='./OB_tide/OB_West_z_Amp.bin',  &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCZGW,FILE='./OB_tide/OB_West_z_Gph.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ...
       IF( East ) THEN
        OPEN(IO_IN_OBCZE,FILE='./OB_tide/OB_East_z_Amp.bin',  &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCZGE,FILE='./OB_tide/OB_East_z_Gph.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ...
       IF( North ) THEN
        OPEN(IO_IN_OBCZN,FILE='./OB_tide/OB_North_z_Amp.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCZGN,FILE='./OB_tide/OB_North_z_Gph.bin',&
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ...
       IF( South ) THEN
        OPEN(IO_IN_OBCZS,FILE='./OB_tide/OB_South_z_Amp.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCZGS,FILE='./OB_tide/OB_South_z_Gph.bin',&
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ...
       IF( West ) THEN
        OPEN(IO_IN_OBCZRW,FILE='./OB_subtide/OB_West_Z.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ENDIF

       IF( East ) THEN
        OPEN(IO_IN_OBCZRE,FILE='./OB_subtide/OB_East_Z.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ENDIF

       IF( North ) THEN
        OPEN(IO_IN_OBCZRN,FILE='./OB_subtide/OB_North_Z.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ENDIF

       IF( South ) THEN
        OPEN(IO_IN_OBCZRS,FILE='./OB_subtide/OB_South_Z.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ENDIF
...
#ifdef OBC_TIDE
       IF( West ) THEN
        OPEN(IO_IN_OBCUW,FILE='./OB_tide/OB_West_u_Amp.bin',  &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCUGW,FILE='./OB_tide/OB_West_u_Gph.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCVW,FILE='./OB_tide/OB_West_v_Amp.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCVGW,FILE='./OB_tide/OB_West_v_Gph.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ...
       IF( East ) THEN
        OPEN(IO_IN_OBCUE,FILE='./OB_tide/OB_East_u_Amp.bin',  &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCUGE,FILE='./OB_tide/OB_East_u_Gph.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCVE,FILE='./OB_tide/OB_East_v_Amp.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCVGE,FILE='./OB_tide/OB_East_v_Gph.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ...
       IF( North ) THEN
        OPEN(IO_IN_OBCUN,FILE='./OB_tide/OB_North_u_Amp.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCUGN,FILE='./OB_tide/OB_North_u_Gph.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCVN,FILE='./OB_tide/OB_North_v_Amp.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCVGN,FILE='./OB_tide/OB_North_v_Gph.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ...
       IF( South ) THEN
        OPEN(IO_IN_OBCUS,FILE='./OB_tide/OB_South_u_Amp.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCUGS,FILE='./OB_tide/OB_South_u_Gph.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCVS,FILE='./OB_tide/OB_South_v_Amp.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCVGS,FILE='./OB_tide/OB_South_v_Gph.bin', &
     &        ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
...
#ifdef OBC_SUBTIDE
       IF( West ) THEN
        OPEN(IO_IN_OBCURW,FILE='./OB_subtide/OB_West_U.bin',           &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCVRW,FILE='./OB_subtide/OB_West_V.bin',           &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ENDIF

       IF( East ) THEN
        OPEN(IO_IN_OBCURE,FILE='./OB_subtide/OB_East_U.bin',           &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCVRE,FILE='./OB_subtide/OB_East_V.bin',           &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ENDIF

      IF( North ) THEN
        OPEN(IO_IN_OBCURN,FILE='./OB_subtide/OB_North_U.bin',          &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCVRN,FILE='./OB_subtide/OB_North_V.bin',          &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ENDIF

      IF( South ) THEN
        OPEN(IO_IN_OBCURS,FILE='./OB_subtide/OB_South_U.bin',          &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCVRS,FILE='./OB_subtide/OB_South_V.bin',          &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
      ENDIF
#endif /*OBC_SUBTIDE*/
      ENDIF
...
! OBC-Temperature & Salinity
      ...
      IF(p_pe==p_io) THEN
       ...
       IF( West ) THEN
        OPEN(IO_IN_OBCTW,FILE='./OB_tracer/OB_West_T.bin',                &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCSW,FILE='./OB_tracer/OB_West_S.bin',                &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ENDIF

       IF( East ) THEN
        OPEN(IO_IN_OBCTE,FILE='./OB_tracer/OB_East_T.bin',                &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCSE,FILE='./OB_tracer/OB_East_S.bin',                &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ENDIF

      IF( North ) THEN
        OPEN(IO_IN_OBCTN,FILE='./OB_tracer/OB_North_T.bin',               &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCSN,FILE='./OB_tracer/OB_North_S.bin',               &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
       ENDIF

      IF( South ) THEN
        OPEN(IO_IN_OBCTS,FILE='./OB_tracer/OB_South_T.bin',               &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
        OPEN(IO_IN_OBCSS,FILE='./OB_tracer/OB_South_S.bin',               &
     &                 ACCESS='SEQUENTIAL',ACTION='READ',FORM='BINARY')
      ENDIF
      ...
      if ( West .AND. have_g_is) then
       if (SpongeWest) then
        ...
        DO i=1,spongew
          ...
          lambda=(s1*rel_ub+s2*rel_ui)/real(spongew)
          IF (lambda.ne.0.) THEN
            lambda=dt/lambda
          ELSE
            lambda=0.0
      ...
      if ( East .AND. have_g_ie ) then
       if (SpongeEast) then
        ...
        DO i=ie,ie-spongee+1,-1
          ...
          lambda=(s1*rel_ub+s2*rel_ui)/real(spongee)
          IF (lambda.ne.0.) THEN
            lambda=dt/lambda
          ELSE
            lambda=0.0
      ...
      if ( North .AND. have_g_js) then
       if (SpongeNorth) then
        ...
        DO j=1,spongen
          ...
          lambda=(s1*rel_vb+s2*rel_vi)/real(spongen)
          IF (lambda.ne.0.) THEN
            lambda=dt/lambda
          ELSE
            lambda=0.0
      ...
      if ( South .AND. have_g_je) then
       if (SpongeSouth) then
        ...
        DO j=je,je-sponges+1,-1
          ...
          lambda=(s1*rel_vb+s2*rel_vi)/real(sponges)
          IF (lambda.ne.0.) THEN
            lambda=dt/lambda
          ELSE
            lambda=0.0
...
! update ORLANSKI OBC predict/relax ts to model fields
      SUBROUTINE UPDATE_ORLANSKI_TS(TTT0,SSS0)
      ...
      ! west OB
      if ( West .AND. have_g_is) then
        TTT(1,2:je1,:)=obc_tw(2:je1,:)*weto(1,2:je1,:)
        SSS(1,2:je1,:)=obc_sw(2:je1,:)*weto(1,2:je1,:)
        ! WRITE(IO_STDOUT,*) ' UPDATE_ORLANSKI_TS  West'
        ! WRITE(IO_STDOUT,*) ' UPDATE_ORLANSKI_TS TTT & SSS'
        ! WRITE(IO_STDOUT,*) ' obc_tw(2:je1,1)', obc_tw(2:je1,1)*weto(1,2:je1,1)
        ! WRITE(IO_STDOUT,*) ' obc_sw(2:je1,1)', obc_sw(2:je1,1)*weto(1,2:je1,1)
      endif

      ! east OB
      if ( East .AND. have_g_ie) then
        TTT(ie,2:je1,:)=obc_te(2:je1,:)*weto(ie,2:je1,:)
        SSS(ie,2:je1,:)=obc_se(2:je1,:)*weto(ie,2:je1,:)
      endif

      ! north OB
      if ( North .AND. have_g_js) then
        TTT(2:ie1,1,:)=obc_tn(2:ie1,:)*weto(2:ie1,1,:)
        SSS(2:ie1,1,:)=obc_sn(2:ie1,:)*weto(2:ie1,1,:)
        ! WRITE(IO_STDOUT,*) ' UPDATE_ORLANSKI_TS  North'
        ! WRITE(IO_STDOUT,*) ' UPDATE_ORLANSKI_TS TTT & SSS'
        ! WRITE(IO_STDOUT,*) ' obc_tn(2:je1,1)', obc_tn(2:ie1,1)*weto(2:ie1,1,1)
        ! WRITE(IO_STDOUT,*) ' obc_sn(2:je1,1)', obc_sn(2:ie1,1)*weto(2:ie1,1,1)
      endif

      ! south OB
      if ( South .AND. have_g_je) then
        TTT(2:ie1,je,:)=obc_ts(2:ie1,:)*weto(2:ie1,je,:)
        SSS(2:ie1,je,:)=obc_ss(2:ie1,:)*weto(2:ie1,je,:)
        ! WRITE(IO_STDOUT,*) ' UPDATE_ORLANSKI_TS  South'
        ! WRITE(IO_STDOUT,*) ' UPDATE_ORLANSKI_TS TTT & SSS'
        ! WRITE(IO_STDOUT,*) ' obc_ts(2:je1,1)', obc_ts(2:ie1,1)*weto(2:ie1,je,1)
        ! WRITE(IO_STDOUT,*) ' obc_ss(2:je1,1)', obc_ss(2:ie1,1)*weto(2:ie1,je,1)
      endif
...
! NorthWest Corner
      if (have_g_is .and. have_g_js) then
         if (WEST .and. NORTH) then
         	do k = 1,ke
         	  if (WETO(1,1,k) .eq. 1) then
               TTT(1,1,k) = (TTT(2,1,k)*WETO(2,1,k) +TTT(1,2,k)*WETO(1,2,k) ) &
                    /( MAX (WETO(2,1,k)+WETO(1,2,k),1.E-3))  
               SSS(1,1,k) =(SSS(2,1,k)*WETO(2,1,k) +SSS(1,2,k)*WETO(1,2,k) )&
                    /( MAX (WETO(2,1,k)+WETO(1,2,k),1.E-3))  
           endif
          enddo
        endif
...
! SouthWest Corner
      if (have_g_is .and. have_g_je) then
      	 if (WEST .and. SOUTH) then
         	do k = 1,ke
      	 	 if (WETO(1,je,k) .eq. 1) then
              TTT(1,je,k) = (TTT(2,je,k)*WETO(2,je,k) +TTT(1,je1,k)*WETO(1,je1,k) ) &
                    /( MAX (WETO(2,je,k)+WETO(1,je1,k),1.E-3))  
              SSS(1,je,k) =(SSS(2,je,k)*WETO(2,je,k) +SSS(1,je1,k)*WETO(1,je1,k) ) &
                    /( MAX (WETO(2,je,k)+WETO(1,je1,k),1.E-3))  
           endif
          enddo
        endif
...
! NorthEast Corner
      if (have_g_ie .and. have_g_js) then
         if (EAST .and. NORTH) then
         	do k = 1,ke
         	  if (WETO(ie,1,k) .eq. 1) then
               TTT(ie,1,k) = (TTT(ie1,1,k)*WETO(ie1,1,k) +TTT(ie,2,k)*WETO(ie,2,k) )&
                    /( MAX (WETO(ie1,1,k)+WETO(ie,2,k),1.E-3))  
               SSS(ie,1,k) = (SSS(ie1,1,k)*WETO(ie1,1,k) +SSS(ie,2,k)*WETO(ie,2,k) )&
                    /( MAX (WETO(ie1,1,k)+WETO(ie,2,k),1.E-3))  
           endif
          enddo
        endif
...
! SouthEast Corner
      if (have_g_ie .and. have_g_je) then
         if (EAST .and. SOUTH) then
         	do k = 1,ke
         	  if (WETO(ie,je,k) .eq. 1) then
              TTT(ie,je,k) = (TTT(ie1,je,k)*WETO(ie1,je,k) +TTT(ie,je1,k)*WETO(ie,je1,k) )&
                    /( MAX (WETO(ie1,je,k)+WETO(ie,je1,k),1.E-3))  
              SSS(ie,je,k) = (SSS(ie1,je,k)*WETO(ie1,je,k) +SSS(ie,je1,k)*WETO(ie,je1,k) )&
                    /( MAX (WETO(ie1,je,k)+WETO(ie,je1,k),1.E-3))  
           endif
         enddo
        endif
      ...
      CALL bounds_exch(TTT)
      CALL bounds_exch(SSS)

      TTT0(:,:,:)=TTT(:,:,:)
      SSS0(:,:,:)=SSS(:,:,:)

      END SUBROUTINE UPDATE_ORLANSKI_TS

! Update ORLANSKI OBC predict uv to model fields
! Which is different from the UPDATE_OBCS_UV on the boundary points
      SUBROUTINE UPDATE_ORLANSKI_UV(UUU0,VVV0)
      ...
      ! west OB
      if ( West .AND. have_g_is) then
        UUU(1,2:je1,:)=obc_uw(2:je1,:)*amsuo(1,2:je1,:)
        if (TanWest) then
           VVV(1,J_start+2:je2,:)=obc_vw(J_start+2:je2,:)*amsue(1,J_start+2:je2,:)
        endif
      endif

      ! east OB
      if ( East .AND. have_g_ie) then
        UUU(ie1,2:je1,:)=obc_ue(2:je1,:)*amsuo(ie1,2:je1,:)
        if (TanEast) then
          VVV(ie,J_start+2:je2,:)=obc_ve(J_start+2:je2,:)*amsue(ie,J_start+2:je2,:)
        endif
      endif
      ...
      if ( North .AND. have_g_js) then
        if (TanNorth) then
          UUU(I_start+2:ie2,1,:)=obc_un(I_start+2:ie2,:)*amsuo(I_start+2:ie2,1,:)
        endif
        VVV(2:ie1,1,:)=obc_vn(2:ie1,:)*amsue(2:ie1,1,:)
      endif

      ! south OB
      if ( South .AND. have_g_je) then
        if (TanSouth) then
          UUU(I_start+2:ie2,je,:)=obc_us(I_start+2:ie2,:)*amsuo(I_start+2:ie2,je,:)
        endif
        VVV(2:ie1,je1,:)=obc_vs(2:ie1,:)*amsue(2:ie1,je1,:)
      endif
...
! NorthWest Corner
      if (have_g_is .and. have_g_js) then
         if (WEST .and. NORTH) then
         	do k = 1, ke
         	  if (WETO(1,1,k) .eq. 1) then
          	   UUU(I_start+1,1,k) = 0.5*(UUU(I_start+2,1,k) +UUU(I_start+1,2,k))*AMSUO(I_start+1,1,k)
         	  	 VVV(1,1,k) = 0.5*(VVV(1,2,k) +VVV(2,1,k) )*AMSUE(1,1,k)
         	  endif
         	enddo
        endif
     endif
...
! SouthWest Corner
      if (have_g_is .and. have_g_je) then
      	 if (WEST .and. SOUTH) then
      	 	do k = 1, ke
      	 	 if (WETO(1,je,k) .eq. 1) then
             UUU(I_start+1,je,k) = 0.5*(UUU(I_start+2,je,k)+UUU(I_start+1,je1,k) )*AMSUO(I_start+1,je,k)
             VVV(1,je1,k) = 0.5*(VVV(2,je1,k) +VVV(1,je2,k) )*AMSUE(1,je1,k)
           endif
          enddo
        endif
     endif
...
! NorthEast Corner
      if (have_g_ie .and. have_g_js) then
         if (EAST .and. NORTH) then
      	 	do k = 1, ke
         	  if (WETO(ie,1,k) .eq. 1) then
               UUU(ie1,1,k) = 0.5*(UUU(ie2,1,k) +UUU(ie1,2,k) )*AMSUO(ie1,1,k)
               VVV(ie,1,k) = 0.5*(VVV(ie1,1,k) +VVV(ie,2,k) )*AMSUE(ie,1,k)
           endif
          enddo
        endif
     endif
...
! SouthEast Corner
      if (have_g_ie .and. have_g_je) then
         if (EAST .and. SOUTH) then
      	 	do k = 1, ke
         	  if (WETO(ie,je,k) .eq. 1) then
              UUU(ie1,je,k) = 0.5*(UUU(ie2,je,k) +UUU(ie1,je1,k) )*AMSUO(ie1,je,k)
              VVV(ie,je1,k) = 0.5*(VVV(ie,je2,k) +VVV(ie1,je1,k) )*AMSUE(ie,je1,k)
          endif
         enddo
        endif
     endif
! ------------------ BOUNDARY CORNERS END --------------------------------
      ...
      RETURN
      END SUBROUTINE UPDATE_ORLANSKI_UV
